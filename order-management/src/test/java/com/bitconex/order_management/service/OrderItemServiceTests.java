...
public class OrderItemServiceTests {
    ...
    void testCreateOrderItem_ShouldThrowException_WhenProductNotFound() {
        when(productService.getProductById(orderItemRequestDTO.getProductId())).thenThrow(new RuntimeException("Product not found"));

        RuntimeException exception = assertThrows(RuntimeException.class, () -> orderItemService.createOrderItem(orderItemRequestDTO));
    ...
    void testCreateOrderItem_ShouldThrowException_WhenOrderNotFound() {
        when(productService.getProductById(orderItemRequestDTO.getProductId())).thenReturn(product);
        when(orderService.getOrderById(orderItemRequestDTO.getOrderId())).thenThrow(new RuntimeException("Order not found"));
    ...
    void testCreateOrderItem_ShouldThrowException_WhenQuantityIsInvalid() {
        OrderItemRequestDTO invalidOrderItemRequestDTO = OrderItemRequestDTO.builder()
                .productId(1L)
                .orderId(1L)
                .quantity(-1)
                .build();

        assertThrows(Exception.class, () -> orderItemService.createOrderItem(invalidOrderItemRequestDTO));
